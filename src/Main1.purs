...
homeScreenFlow = do
  ...
  let hello1 = spy "This is something strange" "Isn't it?"
  _ <- lift $ lift $ pure $ ConfigBridge.setKeyInSharedPrefKeys (show ConfigBridge.WELCOME) "Hi! This is just the beginning of the new end"
  let hello1 = spy "This is something strange" "Isn't it? 2"
  let retrievedValue = ConfigBridge.getKeyInSharedPrefKeys ConfigBridge.WELCOME
      hello = spy "This is the retrieved value from async storage" retrievedValue
  
  output <- lift $ lift $ navigateToScreen $ screen {dummy : "screen1"}
  let hello = spy "Hello world" output -- not gettingOutput
  output <- lift $ lift $ navigateToScreen $ screen {dummy : "screen2"}
  let hello = spy "Hello world 2" output
  pure unit
